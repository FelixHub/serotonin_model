...
data = data.reshape(
    (200000, 1, 64, 64)
)  # we split the 10000 video of 20 frames into 200000 images of 64x64 pixels
...
def main(params_path, model_path):
    ...
    with open(
        "../saved_models/vae-mdrnn/" + model_name + ".params.yml", "w"
    ) as outfile:
    ...
    for epoch in range(nb_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            torch.save(
                model.state_dict(), "../saved_models/vae-mdrnn/" + model_name + ".pt"
            )
