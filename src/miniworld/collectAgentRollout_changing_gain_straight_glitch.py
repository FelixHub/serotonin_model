...
def run_trajectories(nb_trajectories,id_run):
    ...
    for i_trajectories in tqdm(range(nb_trajectories)):
        ...
        gain_change_steps = list(np.sort(np.random.choice(range(1,10),size=nb_changes,replace=False)*10))
    ...
    with open('data/rollout_changing_gain_straight_glitch/agentRollout_observations_'+str(id_run)+'.npy', 'wb') as f:
        np.save(f, trajectories)
    with open('data/rollout_changing_gain_straight_glitch/agentRollout_actions_'+str(id_run)+'.npy', 'wb') as f:
